...
<body>
	<section>
		<div class="pathbox">
			<span class="light">商品管理</span>><span class="darker">新增商品</span>
		</div>
		<div class="addprodlist">
			<form action="/mall/product/update.html" method="post"
				enctype="multipart/form-data" onsubmit="return checkEmpty();">
				<div class="row">
					<span class="addprodinforname">产品型号</span> <span
						class="selectinput"> <span class="selectvalue">${product.basicCategoryModel.name }&nbsp;&nbsp;${product.basicCategoryModel.code }</span>
						<!-- 
                ...
                <input type="text" name="installationCost" value="${product.installationCost }" class="installcost">
                <span>元</span>
            </div> -->
				<div class="row">
					<span class="addprodinforname">产品特点</span> <input type="text"
						name="feature" value="${product.feature }" class="installcost">
				</div>
				<div class="row">
					<span class="addprodinforname">WEB链接</span> <input type="text"
						name="detailUrl" value="${product.detailUrl }" class="installcost">
				</div>
				<div class="row">
					<span class="addprodinforname">APP链接</span> <input type="text"
						name="detailUrlApp" value="${product.detailUrlApp }"
						class="installcost">
				</div>
				<div class="row">
					...
					<div class="upbtnbox">
						<input type="file" name="file" class="file"> <input
							type="button" class="uploadbtn" value="选择图片">
					</div>
					<div class="box">
						<c:if test="${fn:length(productPicList)>0}">
							<c:forEach items="${productPicList }" var="pic"
								varStatus="picStatus">
								<c:if test="${picStatus.index==0}">
									<img src="${pic.url }" class="thumbnail">
								</c:if>
							</c:forEach>
						</c:if>
						<c:if test="${fn:length(productPicList)==0}">
				...
				<div class="row">
					...
					<c:choose>
						<c:when test="${product.status==1 }">
							<input type="radio" name="status" class="putaway" value="1"
								checked="checked">是
               			<input type="radio" name="status" class="putaway"
								value="2">否
	                </c:when>
						<c:otherwise>
							<input type="radio" name="status" class="putaway" value="1">是
                		<input type="radio" name="status" class="putaway"
				...
				<div class="row">
					...
					<c:choose>
						<c:when test="${product.isSpecialOffers}">
							<input type="radio" name="IsSpecialOffers" class="favactive"
								value="1" checked="checked">是
                		<input type="radio" name="IsSpecialOffers"
								class="favactive" value="0">否
	                </c:when>
						<c:otherwise>
							<input type="radio" name="IsSpecialOffers" class="favactive"
								value="1">是
                		<input type="radio" name="IsSpecialOffers"
								class="favactive" value="0" checked="checked">否
	                </c:otherwise>
					</c:choose>
				</div>
				<!-- 添加优惠码相关基础数据  start-->
				<div class="row">
					<span class="addprodinforname">优惠券金额</span> <input type="text"
						name="disMoney" value="${disMoney }" class="installcost">
				</div>
				<div class="row">
					<span class="addprodinforname">分销返利</span> <input type="text"
						name="profitMoney" value="${profitMoney }" class="installcost">
				</div>
				<div class="row">
					<span class="addprodinforname">有效期天数</span> <input type="text"
						name="timeCount" value="${timeCount }" class="installcost">
				</div>
				<!-- 添加优惠码相关基础数据 stop -->
				...
				<div class="row">
					<input type="submit" class="publishbtn" value="发布"> <input
						type="button" class="cancel" value="取消">
				</div>
				<input type="hidden" name="basicCategoryModel.id" id="modelId"
					value="${product.basicCategoryModel.id }" /> <input type="hidden"
...
$(document).ready(function(){
	var modelId=${product.basicCategoryModel.id};
	if(modelId!=null&&modelId!=0){
		_data = {"modelId": modelId,"type":2};
...
$(document).on('change','.file,.uploadbtn',function(){
		var typebtn=$(this);
          ...
          $.ajax({
              type: "POST",
              url: "/mall/product/upload.html?type=1&productId="+productId,
              data: data,
              cache: false,
              contentType: false,    //不可缺
              processData: false,    //不可缺
...
function fillData(data){
	var arrayObj = new Array();　//创建一个数组
 	$.each(data, function(i, item){
		var url;
		$.each(item.pics,function(y,item){
			if(y==0){
				url=item.url
			}
		});
		if(url==null||url==''){
			url='/images/userimg.png';
		}
		var dynamicData='';
		...
		var _temp = ['<tr>',
					    ...
					    '<td>',
					    '<input type="text" name="formstList['+i+'].price" class="hideinput perprice" value="'+item.price+'">',
					    '<input type="hidden" name="formstList['+i+'].id" class="hideinput" value="'+item.id+'"></td>',
					    '<td>',
					    '<input type="text" name="formstList['+i+'].discountPrice" class="hideinput" value="'+item.discountPrice+'"></td>',
	...
	$("#formatT").empty().append(arrayObj.join(''));
}
