...
{ 
if(obj.placeholder.length!=strFormat.length)
    //obj.value='';
...
mvAry[mvAry.length] = '        <th align="center" width="98%" nowrap="nowrap" style="text-align:center;"><select name="calendarYear" id="calendarYear" style="font-size:12px;margin-right:16px;"></select><select name="calendarMonth" id="calendarMonth" style="font-size:12px;';
if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到年时隐藏 “月” 
...
mvAry[mvAry.length] = '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:40px;height:20px;font-size:12px;cursor:pointer"/>';
...
mvAry[mvAry.length] = '        <input name="calendarSure" type="button" id="calendarSure" value="' + Calendar.language["sure"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:40px;height:20px;font-size:12px;cursor:pointer"/>';
...
this.year=parseInt($('#calendarYear').find('option:selected').val());
this.month=parseInt ($('#calendarMonth').find('option:selected').val());
...
$("#calendarYear option").each(function(){
   if(parseInt($(this).val()) === this.year){
...
$("#calendarMonth option").each(function(){
	if(parseInt($(this).val()) === this.month){
		$(this).attr('selected', true);
	}else{
	   $(this).attr('selected', false);
   }
...
if($(this).hasClass('on')){
    $(this).removeClass('on').css('background-color','#efefef');
    //$(this).style.backgroundColor = calendar.colors["td_bg_out"];
}else{
    $(this).addClass('on').css('background-color','#ffcc00');
    //$(this).style.backgroundColor = calendar.colors["sel_bg"];
    //$(this).bgColorTxt = "sel_bg";
}
return false;
}
...
var now = new Date(); 
this.date = (dateObj.placeholder.length > 0) ? new Date(dateObj.placeholder).format(this.dateFormatStyle).toDate(this.dateFormatStyle) : now.format(this.dateFormatStyle).toDate(this.dateFormatStyle) ;//2008-01-29 寒羽枫添加 → 若为空则根据dateFormatStyle初始化日期 
