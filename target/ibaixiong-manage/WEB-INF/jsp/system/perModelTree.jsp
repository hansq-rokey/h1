...
$(function(){
	...
	$('#save_right').bind('click',function(){
		...
		$.ajax({
		   type: "POST",
		   url: "/bbs/role/savePer.html?roleId=${roleId}",
...
function getSelectModel(){
	var models = $("#group_privilege_form").serialize();
	return models;
}
</script>
</head>
<body>
<div>
		<div class="detail_location" style="height:40px;">
			<em class="de_em1"></em>
			<div  class="op_button" style="height:34px;">
				<input id="expandAll" type="button"  class="addrole" style="width:80px;" value="全部展开" />
				<input id="collapseAll" type="button"  class="addrole" style="width:80px;" value="全部收缩" />
				<input id="selectAll" type="button"  class="addrole" style="width:60px;" value="全选" />
				<input id="clearAll" type="button"  class="addrole" style="width:90px;" value="取消全选" />
			</div>
		</div>
		<form id="group_privilege_form">
			<table id="rightTreeGrid" class="easyui-treegrid"
			data-options="url: '/system/role/perTreeSelect.html?roleId=${roleId }',
                 method: 'post',
                 rownumbers: true,
                 idField: 'ID',
                 treeField: 'NAME',
