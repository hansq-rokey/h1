       ...
       <div class="m_content" style="padding-bottom: 100px;">
          <div class="p_title">
            <ul class="p_list">
              ...
              <li class="p_on"> <a href="/mall/product/edit.html?id=${productId }&level=3"><span>3</span></a></li>
             </ul>
          </div>
        <div class="p_imgbox">
          ...
          <div class="p_imgadd ibox" id="uploader-demo">
            ...
            <div class="-view">
              <c:forEach items="${productPicList }" var="item">
                <div class="-view_list">
                 <img src="${item.url }">
                 <span class="-delete" data-id="${item.id }"><img src="../../images_new/_delect.png"></span>
                </div>              
              </c:forEach>
              <span class="-uploader -uploadercolor">最多上传5张</span>
            </div>
          </div>
         <input type="hidden"  id="-delect-" value="" />
           ...
           <tbody>
             ...
             <c:forEach items="${formats }" var="item" varStatus="vs">
              <tr>
                 ...
                 <input type="hidden" name="c_display" value="${item.cDisplay}"/>
                 <input type="hidden" name="b_display" value="${item.bDisplay}"/>
        ...
        <input type="hidden" id="_delectaa" >
      <!--   <p class="p_btn">
  ...
  <div class="p_popup" style="display:none">
    <div class="popup_mian">
     ...
     <div class="popButtonArea">
        ...
        <p class="-uploadercolor -uploadermargin" >关于标识符说明：(长：length；宽：width;高：height；图片:icon)</p>
      </div>
    </div>
...
<!--商品操作-->
<form action="/mall/product/update.html" method="post" id="form3">
     ...
     <li>
       ...
       <li>
           ...
           <div class="roww roww_set">
                ...
                <label class="nuedit1">元/<b id="edit_num" class="edit_num"></b></label>
            </div>
       </li>
       <li>
         <laber>进货价</laber>
         ...
         <laber>单位</laber>
          <div class="rowc_mian setrowc_mian" >
             <div class="rowskin">
               ...
               <span id="selectcontent" class="selectcontent"></span>
       ...
       <li>
         ...
         <div class="roww roww_set">
            <p>
               <input type="checkbox" id="cD" name="cDisplay"  class="selectValue" onchange="selectChecked($(this))" /><span class="Client">C端用户</span>
	           <input type="checkbox" id="bD" name="bDisplay"  class="selectValue" onchange="selectChecked($(this))" /><span class="Client">B端用户</span>
	        </p> 
         </div>
...
function DataArry(item,i,formatId,_check){
   ...
   var content='<li><input type="checkbox" data-formid="'+formatId+'" '+_check+' id="'+item.id+'" ><label>'+$itemName+'</label></li>';
...
$(document).on('click', '.Operation_set', function(event) {
   ...
   var length=that.siblings("input[name='length']").val();
   var width=that.siblings("input[name='width']").val();
   var stock=that.siblings("input[name='stock']").val();
   ...
   var explain=that.siblings("input[name='explain']").val();
   var cDisplay=that.siblings("input[name='c_display']").val();
   var bDisplay=that.siblings("input[name='b_display']").val();
   ...
   var set_se=$(".edit_num").text(unit);//单位
   document.getElementById("set_length").value=length;//长
   document.getElementById("set_width").value=width;//宽
   document.getElementById("set_stock").value=stock;//预留库存
   document.getElementById("set_money").value=price;//市场价
   document.getElementById("set_import").value=productPurchasePrice;//进货价
   document.getElementById("set_zone").value=areaMoney;//区域金额
   document.getElementById("set_explain").value=explain;//说明
   document.getElementById("edit_num").text=unit;//单位
   document.getElementById("selectcontent").text=unit;//单位
   document.getElementById("c_m1").value=unit;//单位
   $("#cD").val(cDisplay);//可见性
   $("#bD").val(bDisplay);//可见性
   document.getElementById("cD").checked=select(cDisplay);//可见性
   document.getElementById("bD").checked=select(bDisplay);//可见性
   $(".template_id").text(templateName);
   /* $(".template_id").attr("data-id",templateId); */
...
$(document).on('click', '._save_btn2', function(event) {
   ...
   var set_length=$("#set_length").val('');//长
   var set_width=$("#set_width").val('');//宽
   var set_stock=$("#set_stock").val('');//预留库存
   var set_money=$("#set_money").val('');//市场价
   var set_import=$("#set_import").val('');//进货价
   var set_zone=$("#set_zone").val('');//区域金额
   var set_explain=$("#set_explain").val('');//说明
   var set_selectcontent=$(".selectcontent").text('');//单位
   var c_display=$("#cD").val('');//可见性
   var b_display=$("#bD").val('');//可见性
